  ...
  return (
    <View style={styles.container}>
      <ScrollView>
        ...
        {carts.length !== 0 ? <View style={{ paddingHorizontal: 16 / 375 * width }}>
          {carts?.map((cart) => {
            return (
              <ItemCartComponent cart={cart} key={cart.id} />
            )
          })}
          ...
          >
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 12 / 375 * width }}>
              <Text style={{ ...TextStyles.bodyNormalTextRegular }}>
                Items (
                {carts.length}
                )
              </Text>
          ...
          >
            <Text style={{ ...TextStyles.bodyLargeTextRegular, color: Colors.neutralGrey }}>NOTHING</Text>
          </View>}
      ...
      <View style={{ alignItems: 'center', paddingBottom: 16 / 375 * width }}>
        <MainButton
          ...
          title={carts.length === 0 ? 'Go back' : 'Check out'}
          onPress={() => { carts.length === 0 && goback() }}
        />
      </View>
...
const ItemCartComponent = ({ cart }) => {
  const dispatch = useDispatch()
  const handAddPress = () => {
    dispatch(cartActions.addProductToCart(cart))
  }
  const handMinusPress = () => {
    dispatch(cartActions.minusProductToCart(cart))
  }
  const handTrashPress = () => {
    dispatch(cartActions.cancleProductToCart(cart))
  }
  return (
    <View style={{
      flexDirection: 'row',
        ...
        >
          ...
          <View style={{ flexDirection: 'row' }}>
            <FastImage
              ...
              resizeMode="contain"
            />
            <TouchableOpacity onPress={() => { handTrashPress() }}>
              <FastImage
                source={trash}
                style={{
                  width: 24 / 375 * width,
